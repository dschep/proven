(function() {
  ...
  const icon = (proof_type, colors) => {
    ...
    if (["generic_web_site", "dns"].includes(proof_type))
      return `<img style="height:12px" src="https://keybase.io/images/paramproofs/services/web/logo_${
        colors ? "full_32" : "black_16@2x"
      }.png">`;
    return `<img style="height:12px" src="https://keybase.io/images/paramproofs/services/${proof_type}/logo_${
      colors ? "full_32" : "black_16@2x"
    }.png">`;
  };
  ...
  const getStyle = () => {
    const styleSheetNames = new Set(
      Array.from(document.styleSheets).map(
        ({ href }) => href && href.split("/").pop()
      )
    );
    let style = "margin-right: 2px;";
    if (
      styleSheetNames.has("nightmode_twitter_core.bundle.css") ||
  ...
  async function getProofsFromStorageOrKeybase(username, service = "twitter") {
    ...
    if (
      fromStorage[key] &&
      fromStorage[key].timestamp > timestamp - 3 * 60 * 60
    ) {
  ...
  async function fetchProofs(username, service = "twitter", retry = 3) {
    ...
    if (resp.status !== 200) {
      ...
      return new Promise(resolve =>
        setTimeout(
          () => resolve(fetchProofs(username, service, retry - 1)),
          1000
        )
      );
    }
  ...
  async function twitterProfiles() {
    const { keybaseBadgeOnly, colors } = await getFromStorage([
      "keybaseBadgeOnly",
      "colors"
    ]);
    let user;
    const element = document.querySelector(
      ".ProfileCard-screenname:not(.proven),.ProfileHeaderCard-screenname:not(.proven)"
    );
    if (element) {
      ...
      for (const { proof_type, nametag, service_url } of proofs) {
        if (
          proof_type === "twitter" ||
          (proof_type !== "keybase" && keybaseBadgeOnly)
        )
          continue;
    ...
    const mobileElement = document.querySelector(
      `${mobileProfileSelector}:not(.proven)`
    );
    if (mobileElement) {
  ...
  async function addTwitterTimelineBadges(element, keybaseBadgeOnly, colors) {
    ...
    if (element.classList.contains("account-group")) {
      ...
      userElement.outerHTML =
        '<span class="UserBadges"></span> ' + userElement.outerHTML;
      target = element.querySelector(".UserBadges");
    } else {
      // mobile.twitter.com
  ...
  async function hackerNews() {
    ...
    for (const element of Array.from(
      document.querySelectorAll(".hnuser:not(.proven)")
    )) {
      element.classList.add("proven");
      ...
      for (const {
        ...
        nametag,
        service_url
      } of proofs.slice().reverse()) {
        if (
          proof_type === "hackernews" ||
          (proof_type !== "keybase" && keybaseBadgeOnly)
        )
          continue;
        element.insertAdjacentHTML(
          "afterend",
          `
          <a href="${service_url}" rel="noreferrer noopener"><span style="${getStyle()}">${icon(
            proof_type,
            colors
          )}</span></a>`
        );
  ...
  if (window.location.host === "news.ycombinator.com") hackerNews();
})();
